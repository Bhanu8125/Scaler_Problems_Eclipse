...
public class HashingShaggyandDistances {
	...
	public static int solve(int[] A) {
        HashMap<Integer,Integer> comm_pair = new HashMap<Integer,Integer>();
		int min_length = A.length;
		for(int i=0;i<A.length;i++)
		{
			if(comm_pair.containsKey(A[i]))
			{
				int length = i - comm_pair.get(A[i]) ;
